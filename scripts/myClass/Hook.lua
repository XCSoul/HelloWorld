...
-- 钩子发射
function Hook:hookLauch()
	self:stopAllActions()
	self._launchFlag = true
	local angle = self:getRotation()
	local move = cc.MoveBy:create(1, ccp(400 * math.sin(math.rad(-angle)), -400 * math.cos(math.rad(-angle))))
	self:runAction(move)
...
function Hook:hookBack()
	...
	local move = cc.MoveTo:create(time, ccp(display.cx, display.cy + 120))
...
function ac_dly_fuc(ob, tk, fuc)
	if not ob then
		log("common ac_dly_func ob is nil value.");
	end
	local ac_dly = CCDelayTime:create(tk);
	local ac_func = CCCallFuncN:create(fuc);
	local ac_seq = CCSequence:createWithTwoActions(ac_dly, ac_func)
