...
-- require有两个好处：1. 按模式加载文件 2.不会重复载入相同的文件。
	...
	self._hook = Hook.new({
		...
		funcBB = function ( goods ) -- 设置钩子刚开始回收的回调
			...
			if index ~= 0 then
				...
				goods:runAction(getSequence({cc.MoveTo:create(time,ccp(display.cx, display.cy +120)),
